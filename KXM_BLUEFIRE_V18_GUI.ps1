# KXM BLUEFIRE v18 GUI
# Windows PowerShell 5.1 compatible. Source intentionally uses ASCII only.
...
$LangFile=Join-Path $PSScriptRoot 'KXM_LANG_V18.json'
New-Item -ItemType Directory -Path $BackupRoot -Force|Out-Null
New-Item -ItemType Directory -Path $LogRoot -Force|Out-Null
$TX=Get-Content -LiteralPath $LangFile -Raw -Encoding UTF8|ConvertFrom-Json
$Script:Lang='EN'

function T($key){$p=$TX.PSObject.Properties[$key+'_'+$Script:Lang];if($null -ne $p){return [string]$p.Value};$q=$TX.PSObject.Properties[$key+'_EN'];if($null -ne $q){return [string]$q.Value};return $key}
...
function Get-Recommendation($h){$c=4;$m=4;if($h.Threads -lt 4){$c=2};if($h.RAM -lt 8){$m=2};$disk='HDD-safe';if($h.SSD){$disk='SSD-ready'};[pscustomobject]@{Cores=$c;RAM=$m;Mode='High Performance';FPS='120 recommended / 240 ceiling optional';Disk=$disk}}
...
function Smart-Optimize{
 ...
 Reg-Dword $mm 'SystemResponsiveness' 0;Reg-Dword $mm 'NetworkThrottlingIndex' 4294967295;Reg-Dword $g 'GPU Priority' 8;Reg-Dword $g 'Priority' 6
 Reg-Dword 'HKLM:\SYSTEM\CurrentControlSet\Control\PriorityControl' 'Win32PrioritySeparation' 38;Reg-Dword 'HKLM:\SYSTEM\CurrentControlSet\Control\Power\PowerThrottling' 'PowerThrottlingOff' 1
...
$sub=New-Object System.Windows.Forms.Label;$sub.Location=New-Object System.Drawing.Point(42,70);$sub.Size=New-Object System.Drawing.Size(850,28);$sub.ForeColor=[System.Drawing.Color]::Silver;$form.Controls.Add($sub)
...
$row=0;$col=0
foreach($item in $toolInfo){$b=New-Object System.Windows.Forms.Button;$b.Text=$item[1];$b.Tag=$item[0];$b.Location=New-Object System.Drawing.Point((15+178*$col),(32+72*$row));$b.Size=New-Object System.Drawing.Size(165,56);$b.BackColor=[System.Drawing.Color]::FromArgb(27,34,44);$b.ForeColor=[System.Drawing.Color]::White;$b.FlatStyle='Flat';$tools.Controls.Add($b);$col++;if($col -ge 3){$col=0;$row++}}

foreach($b in @($tools.Controls)){
 $b.Add_Click({
  $tag=$this.Tag;$hh=Get-Hardware;$rr=Get-Recommendation $hh
  if($tag -eq 'Audit'){$details.Text="HARDWARE AUDIT`r`n`r`nCPU: $($hh.CPU)`r`nCores / Threads: $($hh.Cores) / $($hh.Threads)`r`nRAM: $($hh.RAM) GB`r`nGPU: $($hh.GPU)`r`nHDD: $($hh.HDD)`r`nSSD: $($hh.SSD)`r`nVirtualization: $($hh.Virtualization)`r`n`r`nFREE FIRE`r`nCPU: $($rr.Cores) cores`r`nRAM: $($rr.RAM) GB`r`nMode: High Performance`r`nFPS: $($rr.FPS)"}
  elseif($tag -eq 'BS'){$details.Text="BLUESTACKS ENGINE`r`n`r`nPath: $(Find-BS)`r`nRecommended CPU: $($rr.Cores) cores`r`nRecommended RAM: $($rr.RAM) GB`r`nMode: High Performance`r`n240 FPS: optional ceiling target"}
  elseif($tag -eq 'CPU'){$details.Text='CPU / SCHEDULER`r`n`r`nSmart Optimize uses the KXM game scheduling profile.`r`nNo forced HPET or unsafe kernel tricks.'}
  elseif($tag -eq 'Net'){Apply-Network;$details.Text='NETWORK ENGINE APPLIED`r`n`r`nRSS enabled.`r`nTCP autotuning: normal.'}
  elseif($tag -eq 'Storage'){$details.Text="STORAGE / MEMORY`r`n`r`nHDD: $($hh.HDD)`r`nSSD: $($hh.SSD)`r`nPagefile: Windows managed`r`nMemory Compression: keep enabled`r`nPolicy: $($rr.Disk)"}
  elseif($tag -eq 'BG'){if($hh.HDD -or $hh.RAM -le 8){$policy='KEEP AUTO'}else{$policy='OPTIONAL'};$details.Text="BACKGROUND CONTROL`r`n`r`nSysMain recommendation: $policy`r`n`r`nKXM avoids aggressive service killing on low-memory/HDD systems."}
  elseif($tag -eq 'Bench'){$f=Join-Path $Root ('benchmark_'+(Get-Date -Format 'yyyyMMdd_HHmmss')+'.txt');@('KXM BLUEFIRE v18',"$(Get-Date)","CPU: $($hh.CPU)","RAM: $($hh.RAM) GB","GPU: $($hh.GPU)","HDD: $($hh.HDD)","SSD: $($hh.SSD)")|Set-Content $f -Encoding UTF8;$details.Text=('BENCHMARK SAVED`r`n`r`n'+$f)}
  elseif($tag -eq 'Verify'){$details.Text="VERIFY`r`n`r`nPower: $((powercfg /getactivescheme)-join ' ')`r`nSysMain: $((Get-Service SysMain).StartType)`r`nHAGS override: $((Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers').HwSchMode)"}
  elseif($tag -eq 'Backup'){if(Test-Path $Pointer){$details.Text="BACKUP CENTER`r`n`r`n$((Get-Content $Pointer -Raw -Encoding UTF8).Trim())"}else{$details.Text=(T 'NoBase')}}
 })
}

$ready.Add_Click({$z=Game-Ready;$status.Text=(T 'Ready');$details.Text=((T 'ReadyDone')+'`r`n`r`n'+(if($z.BS){'BlueStacks detected.'}else{'BlueStacks not found in common paths.'}) )})
$smart.Add_Click({$a=[System.Windows.Forms.MessageBox]::Show('Apply the recommended Free Fire profile: 4 CPU cores / 4 GB RAM target / High Performance?','KXM // SMART OPTIMIZE',[System.Windows.Forms.MessageBoxButtons]::YesNo,[System.Windows.Forms.MessageBoxIcon]::Question);if($a -eq [System.Windows.Forms.DialogResult]::Yes){$z=Smart-Optimize;$rr=Get-Recommendation $z;$status.Text='● OPTIMIZED';$details.Text="SMART OPTIMIZE APPLIED`r`n`r`nBlueStacks target: $($rr.Cores) cores / $($rr.RAM) GB`r`nMode: High Performance`r`nFPS ceiling: 240 optional`r`n`r`nRecovery baseline:`r`n$((Get-Content $Pointer -Raw).Trim())"}})
...
$header.Text=T 'Title';$sub.Text=T 'Sub';$status.Text=T 'Ready';$quick.Text='  '+(T 'Quick')+'  ';$tools.Text='  '+(T 'Control')+'  ';$ready.Text=T 'Game';$smart.Text=T 'Smart';$restore.Text=T 'Restore';$details.Text="KXM BLUEFIRE v18`r`n`r`n$(T 'ReadyDone')`r`n`r`nCPU: $($h.CPU)`r`nRAM: $($h.RAM) GB`r`nGPU: $($h.GPU)`r`nStorage: HDD=$($h.HDD) / SSD=$($h.SSD)`r`n`r`n$(T 'Explain')"
